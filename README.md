...
> **约定:每次功能有更新,必须同步更新本章节**(模块状态 + 阶段勾选 + 文末「迭代日志」追加一行)。最后更新:**2026-08-10**。
...
| 2026-08-10 | **AI 助手客户端兼容修复**:生产 HTTP 场景下不再直接依赖 `crypto.randomUUID()`;新增 `randomUUID` → `getRandomValues` → 旧浏览器兼容值的三级回退,始终生成合法 UUID v4;同内容失败重试继续复用 `client_message_id`;前端全量测试 **118/118** 通过、生产构建通过并已部署,健康检查 `ai_shared_store=ready` | 生产 ✅ |
...
- AI 助手客户端已兼容生产 HTTP 场景下缺少 `crypto.randomUUID()` 的浏览器,通过 UUID v4 回退保证消息幂等键可生成；该修复只恢复助手发送能力,**不替代 HTTPS/TLS**,传输加密仍按「安全加固方案.md」推进。
