...
> **约定:每次功能有更新,必须同步更新本章节**(模块状态 + 阶段勾选 + 文末「迭代日志」追加一行)。最后更新:**2026-08-28**。
...
| 系统管理 | P0 | ✅ | 组织架构、岗位权限、人员任职、统一用户管理、操作日志、失效任务改派已完成;组织通讯录按岗位权限开放;提供返回工作平台入口和自适应组织树行 |
...
| 日期 | 本轮内容 | 部署 |
| --- | --- | --- |
| 2026-08-28 | **资金管理与景区经营聚合**：资金流水自动汇总可用余额，授信到期 30 天预警；经营数据中心保留顶部四项 KPI、柱状图和环形图，尾部新增按景区及合计台账；景区配置新增独立酒店页签；资金管理、经营数据中心、战略总览统一以万元展示，门票/酒店/景区原始台账与导出仍以元展示；酒店台账平台列仅显示抖音、美团、携程，酒店名称独立展示 | 待部署 🟡 |
...
| 2026-07-29b | **全站视觉精修 + 核销台账编辑一致性修复**:①建立统一视觉 Token,优化明暗主题、响应式布局、卡片/表格/按钮层级与交互;②修复门票/酒店合法 `0%` 费率被回显为默认 `6%`、输入失焦前未提交、核销金额变化后服务费/待核销未即时展示;③人工到账后调整佣金率时,佣金仍按原逐日明细精算,但到账基数保持人工值;手工佣金金额持续覆盖;④保存后统一由后端按现有逐日计算、逐日舍入和期次递推规则落库,未改底层计算引擎与数据库结构 | 生产 ✅ |
...
| **用户与权限** | 统一账号 CRUD;组织树、岗位目录、任职有效期、治理/外聘任职、岗位权限模板;有效多任职的权限按范围取并集;本人改密/改登录名;手机号脱敏;投资公司董事长、总经理、副总经理仅可跨平台查看、下载和导出;已启用超管为全功能测试账号 |
| **系统管理** | 统一页面提供人员账号、组织通讯录、组织管理、岗位与权限、人员任职、操作日志和 AI 会话审计;提供返回工作平台入口和自适应组织树行 |
...
一个账号可同时拥有多条有效任职,最终权限是这些岗位授权在各自数据范围内的并集。权限授予岗位,不再直接绑定旧角色字符串;任职失效后对应权限立即失效。已启用的超管是全功能测试账号,可执行完整业务与审批工作流;其工作流动作保留真实办理人,并固定快照为「系统治理 / 超级管理员」,且不创建虚假组织、岗位或任职记录。超管账号停用后不具备测试权限。
...
mysql -u root -p sd_publish_scm < backend/migrations/20260828_fund_management.sql                   # 资金管理账户、流水与授信预警
mysql -u root -p sd_publish_scm < backend/migrations/20260828_hotel_config.sql                      # 景区独立酒店配置；必须在资金管理迁移之后执行
```
