...
> **约定:每次功能有更新,必须同步更新本章节**(模块状态 + 阶段勾选 + 文末「迭代日志」追加一行)。最后更新:**2026-07-28**。
...
| 文旅业务(景区核销台账) | — | ✅ | 动态景区目录 → 详情(经营数据卡 + 平台入口 + 折叠台账);门票/酒店 Excel 解析与保存;`biz_scenic_config` 配置默认参数及模块开关;按 `scenic_id` 严格数据隔离;新增景区无需修改前端代码 |
...
| 2026-07-24 | **景区酒店平台核销台账**(抖音/美团/携程多平台;一文件=一期多平台行;核销=基数×90%、服务费=间夜×44、结算=核销+服务费;付款金额隐藏留存、待核销按平台滚动、本期合计行;真实台账 6 期反推验证) | 生产 ✅ |
...
| 2026-07-28c | **景区配置化与多景区接入 V1**:①新增 `biz_scenic_config`,门票/酒店新台账按「请求参数 → 景区配置 → 系统默认」取值并保存实际参数快照,配置变化不影响历史台账;②修复门票解析 `daily_json/positive_count` 透传,保证预览与保存均按「逐日计算、逐日舍入、再汇总」一致;③景区目录改为 `GET /api/v1/scenic-spots` 动态加载,支持 `image_url/ticket_enabled/hotel_enabled/sort_order/enabled`,新增景区无需前端发版;迁移 `20260804_scenic_config.sql`、`20260805_scenic_catalog.sql` | 生产 ✅ (`1493df0`) |
| 2026-08-03 | 台账·服务商佣金率动态可调:①**门票编辑台账行**「付款日期」移到**「付款金额」下方**;②门票+酒店编辑台账行新增**「服务商佣金率」**(仅抖音,默认6%,可动态调整;改后佣金按新费率逐日重算=实收×佣金率−达人−团长,手工改佣金金额则以该值为准);**服务商佣金算法仅对抖音生效**(其它平台佣金恒0);③门票台账新增**「总合计」行**(核销/结算/服务费累加、待核销取末期滚动余额、回款累加)+**表格自适应**(列 `min-width`、去 fixed);新列 `biz_ticket_ledger.commission_rate`、`biz_hotel_ledger.commission_rate`(迁移 `20260803_ledger_commission_rate.sql`,默认0.06,幂等) | 生产 ✅ |
...
| 2026-07-30 | 批量优化:①文旅景区**经营数据卡片实装**(删「当日核销数」;**销售额**=门票+酒店结算金额之和、**核销数**=对账明细订单数、**核销率**=实收/结算为正数订单÷核销数,每景区独立,新端点 `GET /scenic-spots/{id}/metrics`,新列 `positive_count`,迁移 `20260730_ledger_positive.sql`);②发票管理删「发票总数」卡;③**操作审计→操作日志**(改名);④客户档案**新建/编辑/删除仅业务经办+信息维护**(WRITE_ROLES 收窄+前端隐藏按钮) | 生产 ✅ |
...
| **文旅业务**(渠道业务下) | 入口页通过 `GET /api/v1/scenic-spots` 动态加载景区卡片 → 景区详情(平台入口 + **门票/酒店核销数据台账**);景区配置支持费率、酒店服务费算法、图片、排序、启停及模块开关;**Excel 上传/清空/预览**;**按 `scenic_id` 严格数据隔离**(接口只返本景区数据,`WHERE scenic_id`);新增景区无需修改前端代码 |
...
mysql -u root -p sd_publish_scm < backend/migrations/20260803_ledger_commission_rate.sql # 门票/酒店台账服务商佣金率 commission_rate(默认0.06)
mysql -u root -p sd_publish_scm < backend/migrations/20260804_scenic_config.sql         # 景区台账默认配置 biz_scenic_config(不迁移、不重算历史台账)
mysql -u root -p sd_publish_scm < backend/migrations/20260805_scenic_catalog.sql        # 动态景区目录展示字段、模块开关及排序
```
...
- 当前生产版本:`1493df0`(景区台账配置化 + 动态景区目录 V1);`20260805_scenic_catalog.sql` 已执行,后端健康检查和景区目录接口已验收通过。
