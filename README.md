...
| 2026-07-25 | **前端体验优化 · 通用列表组件 `ProTable`**(配置化列 + 内置关键词搜索 + 客户端分页 + **首屏骨架屏** + **加载失败重试态** + 空态,`defineExpose(reload)`);首个试点页 `invoice` 迁移(列配置化、统计卡加 `:xs/:sm` 响应式断点、税号格式校验);为 contract/approval/customer/users 铺开做准备 | 生产 ✅ |
