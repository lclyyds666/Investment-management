<template>
  <form class="composer" data-testid="message-composer" aria-label="发送问题" @submit.prevent="submit">
    ...
    <div class="composer__actions">
      ...
      <el-button v-else class="composer__button" type="primary" native-type="submit" :disabled="!normalizedDraft" aria-label="发送问题">发送</el-button>
    </div>
  </form>
</template>
...
function submit() {
  if (!normalizedDraft.value || props.generating) return
  emit('submit', normalizedDraft.value)
  draft.value = ''
}
