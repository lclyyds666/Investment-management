    ...
    <el-dialog v-model="editVisible" title="编辑台账行" width="480px">
      <el-form label-width="120px" v-if="editRow">
        ...
        <el-form-item v-if="editForm.platform === '抖音'" label="服务商佣金率">
          <el-input-number v-model="editForm.commissionRatePct" :min="0" :max="100" :precision="2" :step="0.5" controls-position="right" style="width: 100%" @change="onCommissionRateChange" />
...
import {
  ...
  previewTicketRow, updateTicketRow, deleteTicketRow, fetchTicketLedgerExportBlob,
...
async function onCommissionRateChange() {
  if (!editRow.value || editForm.platform !== '抖音') return
  editForm.commissionEdited = false
  editForm.hexiaoEdited = false
  editForm.jinyingEdited = false
  const sequence = ++commissionPreviewSequence
  try {
    const result = await previewTicketRow(
      props.scenicId,
      editRow.value.id,
      buildEditPayload()
    )
