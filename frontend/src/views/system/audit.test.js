...
async function mountRows(items, reassignmentAudits = []) { api.getAuditMeta.mockResolvedValue({ actions: [], modules: [] }); api.listAuditLogs.mockResolvedValue({ total: items.length, items }); workflowApi.listReassignmentAudits.mockResolvedValue(reassignmentAudits); const wrapper = mount(AuditView, { global: { stubs } }); await flushPromises(); return wrapper }
