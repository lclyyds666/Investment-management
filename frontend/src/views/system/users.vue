<template>
  <ProTable ref="tableRef" title="人员账号" :fetch="fetchUsers" :columns="columns" row-key="id">
    ...
    <template #actions="{ row }"><el-button size="small" @click="openEdit(row)">编辑</el-button><el-button size="small" @click="router.push({ name: 'SystemAssignments', query: { user_id: row.id } })">任职</el-button><el-button size="small" @click="resetUserPassword(row.id)">重置密码</el-button></template>
  </ProTable>
  <el-dialog v-model="dialog.visible" :title="dialog.id ? '编辑账号' : '新建账号'" width="480px"><el-form :model="dialog.form" label-width="86px"><el-form-item label="登录账号"><el-input v-model="dialog.form.username" :disabled="Boolean(dialog.id)" /></el-form-item><el-form-item label="姓名"><el-input v-model="dialog.form.full_name" /></el-form-item><el-form-item v-if="!dialog.id" label="初始密码"><el-input v-model="dialog.form.password" type="password" /></el-form-item><el-form-item v-else label="启用"><el-switch v-model="dialog.form.is_active" /></el-form-item></el-form><template #footer><el-button @click="dialog.visible=false">取消</el-button><el-button type="primary" @click="save">保存</el-button></template></el-dialog>
</template>
<script setup>
...
const columns = [{ prop:'full_name', label:'姓名', minWidth:100 }, { prop:'username', label:'登录账号', minWidth:120 }, { label:'任职身份', slot:'assignments', minWidth:280 }, { label:'电子签名', slot:'signature', width:100 }, { label:'操作', slot:'actions', width:220 }]
...
async function openCreate(){ dialog.id=null; dialog.form={ username:'', full_name:'', password:'' }; dialog.visible=true }
function openEdit(row){ dialog.id=row.id; dialog.form={ username:row.username, full_name:row.full_name, is_active:row.is_active }; dialog.visible=true }
async function save(){ if(dialog.id) await updateUser(dialog.id,{ full_name:dialog.form.full_name,is_active:dialog.form.is_active }); else await createUser(dialog.form); dialog.visible=false; reload() }
