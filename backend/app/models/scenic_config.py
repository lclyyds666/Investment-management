...
class ScenicConfig(Base):
    ...
    scenic_name: Mapped[str] = mapped_column(String(100), nullable=False, comment="景区名称")
    ...
    ticket_rate_hexiao: Mapped[Decimal] = mapped_column(
        Numeric(6, 4), nullable=False, default=Decimal("0.9000"), comment="门票默认核销率"
    )
    ticket_rate_settle: Mapped[Decimal] = mapped_column(
        Numeric(6, 4), nullable=False, default=Decimal("0.9400"), comment="门票默认结算费率"
    )
    ticket_commission_rate: Mapped[Decimal] = mapped_column(
        Numeric(6, 4), nullable=False, default=Decimal("0.0600"), comment="门票默认服务商佣金率"
    )
